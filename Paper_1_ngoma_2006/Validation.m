...
% As h = h1,
% this means that we are only considering Fluid 1
...
phi_star_ana = dimElecPotential(K, h1_star, zeta1_star, zeta2_star, ...
    N, resultType);
...
u1_star_ana = ((G1./2) .* (1 - (ymesh.^2))) - ...
    (2.*G2.*E.*zeta1_star./(K^2)).*(1 - (phi_star_ana ./ zeta1_star));
...
scatter(ymesh(1:9:N), u1_star_ana(1:9:N), 50, "black", "filled")
hold off
...
hold on
plot(ymesh, phi_star, LineWidth=1.75, LineStyle="-")
scatter(ymesh(1:9:N), phi_star_ana(1:9:N), 50, "black", "filled")
hold off
legend('Numerical', 'Analytical', Location='best')
xlabel('Dimensionless Distance Between Plates')
ylabel('Dimensionless Potential')
