...
% Fluid Velocity
sol.u_star = velocityProfile(C0, C1, C2, data.alpha, h1_star, zeta1_star, zeta2_star, sol.phi_star, N, sol.y_star);
...
sol.tau_int_star = interfaceShearStress(C0, C1, C2, data.alpha, h1_star, zeta1_star, zeta2_star);
...
function [ output ] = velocityProfile(C0, C1, C2, alpha, h1_star, zeta1_star, zeta2_star, phi_star, N, ymesh)

a2 = -C1 * zeta1_star;

den = 1 + (h1_star * (alpha - 1));

b1 = (1/den) * ((alpha*C2*(h1_star^2)) + (C2/2) - ...
    ((h1_star^2)*(C0+C2)/2) + (C1*(zeta1_star - zeta2_star)));

a1 = (alpha*b1) + (C0*h1_star) - (alpha*C2*h1_star);
...
function [ output ] = interfaceShearStress(C0, C1, C2, alpha, h1_star, zeta1_star, zeta2_star)
